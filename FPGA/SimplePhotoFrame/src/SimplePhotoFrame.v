...
module simple_photo_frame (
	...
	// ---------------------------------------------------------
	Gowin_rPLL u_rpll (
		.clkout					( w_clk108m						),		//	108MHz
		.clkoutp				( w_clk108m_n					),		//	108MHz (inverted)
		.clkin					( clk27m						)		//	27MHz
	);
	...
	//	リセット生成
	// ---------------------------------------------------------
	always @( posedge w_clk108m ) begin
		ff_reset_bus		<= 1'b0;
		ff_reset_spi		<= 1'b0;
		ff_reset_gp1		<= 1'b0;
		ff_reset_gp2		<= 1'b0;
	...
	display_controller u_display_controller (
		...
		.sdram_address_valid	( w_sdram_display_valid			),
		.sdram_address_ready	( w_sdram_display_ready			),
